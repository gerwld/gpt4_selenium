...
Created article should be detailed, with minimum 2000 words in it. \n\
...
Put images placeholders inside post where they have to be, with detailed desription of what they should contain. For example - [Chess possition Fisher vs Spassky, 1992]\n\
...
Article title: '
        ...
        if not os.path.exists(PATH_TO_TILES):
            print(
                f'{C_RED}chatGPT-create-by-title: {PATH_TO_TILES} do not exist.{C_RED.OFF}')
        else:
                    ...
                    # пінгування щоб обійти ліміт і обрив генерації (0 щоб виключити)
                        ...
                        if isPostValid(str(answer).strip()):
                            ...
                            delPhFinalPost = delPhrasesSoup(
                                str(postSoup.find('article')).strip(), DEL_PHRASES)
